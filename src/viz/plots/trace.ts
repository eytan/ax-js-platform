...
function renderOptimizationTraceStatic(
  ...
  // Running best
  let best = yVals[0];
  const bestSoFar = yVals.map((y): number => {
  ...
  const sx = (i: number): number => margin.left + (i / Math.max(1, n - 1)) * pw;
  ...
  svg.append(
    svgEl("line", {
      x1: margin.left + pw - 120,
      x2: margin.left + pw - 100,
      y1: margin.top + 12,
      y2: margin.top + 12,
      stroke: "#444",
      "stroke-width": 2.5,
