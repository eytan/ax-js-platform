...
describe("buildKernel", () => {
  ...
  it("builds recursive Scale(Product(RBF, Categorical))", () => {
    const state: KernelState = {
      ...
      base_kernel: {
        type: "Product",
        kernels: [
          { type: "RBF", lengthscale: [1.0], active_dims: [0] },
          { type: "Categorical", lengthscale: [1.0], active_dims: [1] },
        ],
      },
    };
    const kernel = buildKernel(state);
  ...
  it("builds Additive kernel", () => {
    const state: KernelState = {
      type: "Additive",
      kernels: [
        { type: "RBF", lengthscale: [1.0, 1.0] },
        { type: "RBF", lengthscale: [2.0, 2.0] },
      ],
    };
    const kernel = buildKernel(state);
    const K = kernel.compute(x, x);
