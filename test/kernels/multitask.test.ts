...
describe("IndexKernel", () => {
  it("computes task covariance B = W@W^T + diag(exp(log_var))", () => {
    ...
    expect(K.get(0, 0)).toBeCloseTo(2, 10); // B[0,0] = 1 + 1
    expect(K.get(1, 1)).toBeCloseTo(2, 10); // B[1,1] = 1 + 1
    expect(K.get(0, 1)).toBeCloseTo(0, 10); // B[0,1] = 0
  });
  ...
  it("handles rank-1 covariance", () => {
    ...
    expect(K.get(0, 0)).toBeCloseTo(2, 10); // B[0,1] = 1*2 = 2
  });
});
