...
import type { SearchSpaceParam, KernelState, ExperimentState } from "../src/models/types.js";
...
describe("gaussLegendre01", () => {
  ...
  it("integrates exp(-x²) on [0,1] with 32 nodes to high precision", () => {
    ...
    expect(sum).toBeCloseTo(0.7468241328, 10);
  });
});
...
describe("RbfDimIntegrator", () => {
  ...
  it("marginal matches numerical quadrature for various ℓ and c", () => {
    for (const ell of [0.1, 0.3, 0.5, 1.0, 2.0]) {
      const integrator = new RbfDimIntegrator(ell);
      for (const c of [0.0, 0.2, 0.5, 0.8, 1.0]) {
...
describe("CategoricalDimIntegrator", () => {
  it("marginal is correct for 2 categories", () => {
    ...
    expect(integrator.marginal(0)).toBeCloseTo(
      (1 + Math.exp(-2)) / 2,
      10,
    );
  });
  ...
  it("cross(a, a) matches explicit computation", () => {
    ...
    expect(integrator.cross(0, 0)).toBeCloseTo(
      (1 + (K - 1) * e2) / K,
      10,
    );
  });
  ...
  it("cross(a, b) with a ≠ b matches explicit computation", () => {
    ...
    const e2 = e1 * e1;
    // W(a, b) = (1/K)[2e + (K-2)e²]
    expect(integrator.cross(0, 1)).toBeCloseTo(
      (2 * e1 + (K - 2) * e2) / K,
      10,
    );
  });
...
describe("QuadratureDimIntegrator", () => {
  ...
  it("matches Simpson's rule for RBF kernel function", () => {
    const ell = 0.3;
    const rbfKernel = (x: number, c: number) =>
  ...
  it("handles warped RBF (Kumaraswamy CDF)", () => {
    ...
    const warpedKernel = (x: number, c: number) => {
...
describe("MaternDimIntegrator", () => {
  ...
  for (const nu of [0.5, 1.5, 2.5] as (0.5 | 1.5 | 2.5)[]) {
    ...
    describe(`nu = ${nu}`, () => {
      it("marginal matches Simpson's rule", () => {
        for (const ell of [0.1, 0.3, 0.5, 1.0]) {
...
describe("extractKernelComponents", () => {
  const params2d: SearchSpaceParam[] = [
  ...
  it("extracts components for Scale(RBF)", () => {
    ...
    expect(result![0].outputscale).toBe(2.0);
  });
  ...
  it("extracts components for Scale(Matern)", () => {
    ...
    expect(result!.length).toBe(1);
    expect(result![0].activeDims).toEqual([0, 1]);
    expect(result![0].outputscale).toBe(1.0);
  });

  it("extracts Product(RBF, Categorical) components", () => {
    const params: SearchSpaceParam[] = [
      { name: "x0", type: "range", bounds: [0, 1] },
      { name: "x1", type: "range", bounds: [0, 1] },
    ...
    const kernel: KernelState = {
      ...
      base_kernel: {
        ...
        kernels: [
          ...
          { type: "Categorical", lengthscale: [1.0], active_dims: [2] },
        ],
      },
    };
    ...
    expect(result![0].outputscale).toBe(3.0);
  });

  it("extracts Additive components with disjoint dims", () => {
    const params4d: SearchSpaceParam[] = [
      { name: "x0", type: "range", bounds: [0, 1] },
      { name: "x1", type: "range", bounds: [0, 1] },
    ...
    expect(result![0].activeDims).toEqual([0, 1]);
    expect(result![0].outputscale).toBe(2.0); // parent 1.0 × component 2.0
    expect(result![1].activeDims).toEqual([2, 3]);
    expect(result![1].outputscale).toBe(3.0);
  });
  ...
  it("returns null for Additive with overlapping dims", () => {
    const params3d: SearchSpaceParam[] = [
      { name: "x0", type: "range", bounds: [0, 1] },
      { name: "x1", type: "range", bounds: [0, 1] },
      { name: "x2", type: "range", bounds: [0, 1] },
    ];
...
describe("computeAnalyticSobolIndices", () => {
  ...
  function buildSyntheticGP(opts: {
    ...
    const alpha = new Float64Array(
      opts.weights.map((w) => w / opts.outputscale),
    );
  ...
  it("single training point at center → equal first-order for equal ℓ", () => {
    ...
    expect(Math.abs(result.firstOrder[0] - result.firstOrder[1])).toBeLessThan(
      1e-10,
    );
  ...
  it("structural invariants: S ≥ 0, ST ≥ S, Σ S_i ≤ 1", () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      expect(result.totalOrder[i]).toBeGreaterThanOrEqual(
        result.firstOrder[i] - 1e-10,
      );
    }
  ...
  it("constant function → zero indices", () => {
    ...
    const result = computeAnalyticSobolIndices(
      ...
      [[0.5, 0.5]],
      extractKernelComponents(
        { type: "RBF", lengthscale: [0.3, 0.3], outputscale: 1.0 },
        [
          { name: "x0", type: "range", bounds: [0, 1] },
          { name: "x1", type: "range", bounds: [0, 1] },
        ],
  ...
  describe("branin_rbf fixture", () => {
    ...
    it("structural invariants hold", () => {
      ...
      for (let i = 0; i < sens.firstOrder.length; i++) {
        ...
        expect(sens.totalOrder[i]).toBeGreaterThanOrEqual(
          sens.firstOrder[i] - 0.01,
        );
      }
    });
  });
...
describe("MultiTaskGP analytic sensitivity", () => {
  it("computes per-task sensitivity for branin_multitask", () => {
    ...
    const dataKernelType =
      ms.data_kernel?.type === "Scale"
        ? ms.data_kernel.base_kernel?.type
        : ms.data_kernel?.type;
...
describe("mixed kernel (Product(RBF, Categorical))", () => {
  it("hartmann_mixed: analytic matches MC", () => {
    ...
    if (analyticSens.numEvaluations === 0) {
      ...
      for (let i = 0; i < analyticSens.firstOrder.length; i++) {
        expect(
          Math.abs(analyticSens.firstOrder[i] - mcSens.firstOrder[i]),
        ).toBeLessThan(0.05);
      }
    }
...
describe("rbfGeneralizedCross", () => {
  it("matches standard cross when ℓ₁ = ℓ₂", () => {
    ...
    for (const [a, b] of [
      ...
      expect(rbfGeneralizedCross(a, b, ell, ell)).toBeCloseTo(
        integrator.cross(a, b),
        10,
      );
    }
  });
...
describe("computeEnsembleAnalyticSobol", () => {
  it("single-model ensemble equals standard analytic", () => {
    ...
    const ls = [0.3, 0.5];
    const os = 1.5;
    const alpha = new Float64Array([1.0, -0.5, 0.8]);
    const trainX = [
      [0.2, 0.3],
      [0.5, 0.7],
      [0.8, 0.1],
    ...
    const paramSpecs: SearchSpaceParam[] = paramNames.map((name) => ({
      name,
      type: "range" as const,
      bounds: [0, 1] as [number, number],
    }));
    ...
    const standard = computeAnalyticSobolIndices(
      alpha,
      trainX,
      components,
      0.5,
  ...
  it("two-model ensemble with different ℓ: structural invariants", () => {
    const models: EnsembleSubModelInfo[] = [
      {
        alpha: new Float64Array([1.0, -0.3]),
        trainXNorm: [[0.2, 0.4], [0.7, 0.6]],
        meanConstant: 0.0,
  ...
  it("ensemble with equal ℓ across models matches pooled single model", () => {
    ...
    const m1: EnsembleSubModelInfo = {
      alpha: new Float64Array([1.0, -0.5]),
      trainXNorm: [[0.2, 0.3], [0.8, 0.1]],
      meanConstant: 0.5,
      lengthscales: ls,
      outputscale: os,
    };
    ...
    const pooledAlpha = new Float64Array([
      ...Array.from(m1.alpha),
      ...Array.from(m2.alpha),
    ]);
    ...
    const paramSpecs: SearchSpaceParam[] = ["x0", "x1"].map((name) => ({
      name,
      type: "range" as const,
      bounds: [0, 1] as [number, number],
    }));
    ...
    const pooled = computeAnalyticSobolIndices(
      pooledAlpha,
      pooledTrainX,
      components,
      pooledMean,
