...
describe("Predictor ↔ Ax Adapter parity", () => {
  ...
  for (const entry of axFixtures) {
    ...
    const flatMeans: number[] = rawMeans === null ? [] :
      typeof rawMeans === "object" && !Array.isArray(rawMeans)
        ? Object.values(rawMeans as Record<string, number[]>).flat()
        : Array.isArray(rawMeans[0])
          ? (rawMeans as number[][]).flat()
          : (rawMeans as number[]);
    ...
    describe(`fixture: ${entry.name}`, () => {
      ...
      if (isAllTasks) {
        ...
        it("predictions match for all tasks", () => {
          ...
          const expectedMeans = fixture.test.expected.mean as Record<
            string,
            number[]
          >;
          const expectedVars = fixture.test.expected.variance as Record<
            string,
            number[]
          >;
        ...
        for (let k = 0; k < expectedMeans.length; k++) {
          const outcomeName =
            fixture.experiment.outcome_names?.[k] ?? `y${k}`;
        ...
        it("mean matches", () => {
          ...
          expectAllClose(
            predictions[outcomeName].mean,
            fixture.test.expected.mean as number[],
            TOLERANCE,
            "mean",
          );
        });
        ...
        it("variance matches", () => {
          ...
          expectAllClose(
            predictions[outcomeName].variance,
            fixture.test.expected.variance as number[],
            TOLERANCE,
            "variance",
          );
        });
      ...
      if (fixture.test.expected_relative && fixture.experiment.status_quo) {
        ...
        const isPerOutcome =
          !("mean" in expectedRel) ||
          (typeof expectedRel.mean === "object" &&
            !Array.isArray(expectedRel.mean));
